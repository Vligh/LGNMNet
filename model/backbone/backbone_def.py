...
from backbone.MergeModel import merge_Net
...
class BackboneFactory:
    ...
    def get_backbone(self):
        ...
        elif self.backbone_type == 'MnasNet':
            feat_dim = self.backbone_param['feat_dim'] # dimension of the output features, e.g. 512.
            num_class = self.backbone_param['num_class'] # dimension of the output features, e.g. 512.
            pretrained = self.backbone_param['pretrained'] # dimension of the output features, e.g. 512.
            image_size = self.backbone_param['image_size'] # height of the feature map before the final features.
            backbone = MnasNet(num_classes=num_class, embedding_size=feat_dim, pretrained=pretrained)
        elif self.backbone_type == 'MobileNet':
            feat_dim = self.backbone_param['feat_dim'] # dimension of the output features, e.g. 512.
            num_class = self.backbone_param['num_class'] # dimension of the output features, e.g. 512.
            pretrained = self.backbone_param['pretrained'] # dimension of the output features, e.g. 512.
            image_size = self.backbone_param['image_size'] # height of the feature map before the final features.
            backbone = MobileNet(num_classes=num_class, embedding_size=feat_dim, pretrained=pretrained)
        elif self.backbone_type == 'ShuffleNet':
            feat_dim = self.backbone_param['feat_dim'] # dimension of the output features, e.g. 512.
            num_class = self.backbone_param['num_class'] # dimension of the output features, e.g. 512.
            pretrained = self.backbone_param['pretrained'] # dimension of the output features, e.g. 512.
            image_size = self.backbone_param['image_size'] # height of the feature map before the final features.
            backbone = ShuffleNet(num_classes=num_class, embedding_size=feat_dim, pretrained=pretrained)
        elif self.backbone_type == 'MergeModel':
            feat_dim = self.backbone_param['feat_dim'] # dimension of the output features, e.g. 512.
            num_class = self.backbone_param['num_class'] # dimension of the output features, e.g. 512.
            pretrained = self.backbone_param['pretrained'] # dimension of the output features, e.g. 512.
            image_size = self.backbone_param['image_size'] # height of the feature map before the final features.
            backbone = merge_Net(init_weights=False, batch_size=32, dataset="SAMM", mode="Macro")
